...
> **FDE adaptation:** This vendored copy enables only local pattern warnings on
> code edits. FDE disables the upstream SessionStart SDK bootstrap and the
> Stop/commit/push LLM reviewers because those features install a dependency,
> spend model budget, and transmit diffs or related source files. The complete
> upstream implementation remains vendored and pinned for auditability, but its
> outbound layers are not registered in `hooks/hooks.json`.
